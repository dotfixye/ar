...
---------------------------------------------------*/
    ...
    window.ScrollEffects = function() {
        ...
        if (isMobile()) {
            ...
            $(
                ".smooth-scroll main, .infinity-lists-holder, .infinity-lists-wrapper, nav, #canvas-slider, #showcase-slider-holder, #showcase-carousel-holder, #hero.has-image #hero-caption, #hero-image-wrapper, .thumb-container, .thumb-page, .next-project-image-wrapper"
            ).css({
                height: winHeight
            });
        ...
        $("body").waitForImages({
            finished: function() {
                ...
                gsap.utils.toArray(".slowed-pin").forEach((slowedPin) => {
                    ...
                    const slowedImagesLeft =
                        slowedPin.querySelectorAll(".slowed-image-left");
                    const slowedImagesRight = slowedPin.querySelectorAll(
                        ".slowed-image-right"
                    );
                    ...
                    if (slowedText.offsetHeight > window.innerHeight) {
                        ...
                        gsap.to(slowedText, {
                            scrollTrigger: {
                                ...
                                pin: true,
                                start: "top +" +
                                    (window.innerHeight / 2 - slowedText.offsetHeight / 2),
                                end: "+=" +
                                    (slowedPin.offsetHeight -
                                        (window.innerHeight / 2 + slowedText.offsetHeight / 2)),
                            },
                        });
                    }
                    ...
                    if (!isMobile()) {
                        $(".news-panel .panel .panel-image").mouseenter(function(e) {
                            ...
                            gsap.to("#ball", {
                                ...
                                borderWidth: "2px",
                                scale: 1.2,
                                borderColor: $this.parents(".panel").data("color"),
                                backgroundColor: $this.parents(".panel").data("color"),
                            });
                            gsap.to("#ball-loader", {
                                duration: 0.2,
                                borderWidth: "2px",
                            ...
                            $("#ball").append(
                                '<p class="first">' +
                                $this.parents(".panel").data("firstline") +
                                "</p>" +
                                "<p>" +
                                $this.parents(".panel").data("secondline") +
                                "</p>"
                            );
                        });
        ...
        if ($("body").hasClass("hero-below-caption")) {
            var heroImageParallax = gsap.to(".parallax-scroll-image", {
                duration: 1,
                top: "10%",
                scale: 1,
                opacity: 1,
        ...
        var nextProjectImageScene = new ScrollMagic.Scene({
            ...
            .addTo(controller);
        if ($("body").hasClass("hero-below-caption")) {
            var ProjectNavHeight =
                $("#project-nav").outerHeight() + $("footer").outerHeight();
            ...
            if ($("#itemsWrapper").hasClass("webgl-fitthumbs")) {
                if ($("#itemsWrapper").length > 0) {
                    ...
                    if ($("#itemsWrapperLinks").length > 0) {
                        ...
                        for (
                            let idxCaption = 0; idxCaption < thumbsLink.length; idxCaption++
                        ) {
                    ...
                    imagesLoaded(
                        ...
                        (instance) => {
                            let images = [];
                            for (
                                var i = 0, imageSet = {}; i < instance.elements.length; i++
                            ) {
                                let image = {
            ...
            if ($("#itemsWrapper").hasClass("scale-fitthumbs")) {
                ...
                function addListeners(tile, page, url) {
                    url.addEventListener("click", function() {
                        ...
                        gsap.to(
                            "#hero, #show-filters, .item, .item-caption-wrapper, #page-nav, footer, .fadeout-element", {
                                duration: 0.3,
                                opacity: 0,
                                delay: 0,
                                ease: Power2.easeIn
